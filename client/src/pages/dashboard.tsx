...
export default function DashboardPage() {
  ...
  const currentSummary = auditData ? {
    ...
    runway: auditData.runway ?? mockFinancials.summary.runway
  } : mockFinancials.summary;
  ...
  const currentHistory = (auditData && auditData.history) ? auditData.history : mockFinancials.history;
  ...
  return (
    <Layout>
      <div className="space-y-8 pb-10 font-sans">
        ...
        {auditData?.warning && (
          <Alert variant="default" className="bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-900/30">
            ...
            {currentLeaks.map((leak: any) => (
              <Card key={leak.id} className="border-primary/20 shadow-lg shadow-primary/5 hover:border-primary/40 transition-all">
                <CardContent className="p-6 space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{leak.description}</p>
